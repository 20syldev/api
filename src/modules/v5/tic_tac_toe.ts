...
function playMove(
    ...
    if (!g.players.includes(u)) {
        if (g.players.length >= 2) throw new Error('Game is full, you can only watch.');
        g.players.push(u);
    }

    if (moves.length > 0 && moves[moves.length - 1]!.username === u) {
    ...
    const play: TicTacToeMove = { username: u, move, session };
    moves.push(play);
    g.last = now;
